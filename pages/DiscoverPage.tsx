...
export default function DiscoverPage() {
  ...
  const userAvatarSrc = user ? user.photo || getAvatarUrl(user.gender, user.id) : undefined;
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {profiles.map((profile) => (
              <div
                ...
                onClick={() => router.push(`/profile/${profile.user}`)}
              >
                {/* Cover Area */}
                ...
                <div className="px-5 pb-5 -mt-14 flex flex-col items-center flex-1">
                  <Avatar className="w-28 h-28 border-[6px] border-white shadow-xl mb-4 bg-background ring-1 ring-border group-hover:scale-105 transition-transform duration-300">
                    <AvatarImage src={profile.photo || `https://api.dicebear.com/7.x/avataaars/svg?seed=${profile.user}`} className="object-cover" />
                  ...
                  <div className="grid grid-cols-4 gap-2 w-full mt-auto">
                    ...
                    <button
                      onClick={(e) => { e.stopPropagation(); router.push(`/chat/${profile.user}`); }}
