...
export default function GenderSelectionPage() {
  ...
  const handleSelect = async () => {
    ...
    try {
      ...
      router.replace('/language'); // next: language selection
    } catch (error: unknown) {
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-card rounded-2xl shadow-sm border border-border p-4 space-y-4">
          {genders.map((gender) => {
            ...
            return (
              <button
                ...
                className={`w-full p-4 rounded-xl border-2 flex items-center gap-4 transition ${isSelected
          ...
          <button
            ...
            className={`w-full mt-6 py-3 rounded-xl font-semibold transition ${!selectedGender || isLoading
