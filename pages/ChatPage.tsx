  ...
  return (
    <div className="flex items-center gap-2 min-w-[140px] bg-black/5 p-2 rounded-full">
      <button onClick={togglePlay} className="w-8 h-8 bg-primary text-white rounded-full flex items-center justify-center shrink-0">
        {isPlaying ? <Pause size={16} color="currentColor" /> : <Play size={16} color="currentColor" />}
      </button>
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-50">
      ...
      <div className="bg-white p-3 shadow-sm flex items-center justify-between z-10">
        <div className="flex items-center gap-3">
          ...
          <div className="relative">
            ...
            <span
              className={`absolute bottom-0 right-0 w-3 h-3 border-2 border-white rounded-full ${presence === 'busy' ? 'bg-red-500' : 'bg-green-500'
                }`}
            ></span>
          </div >
          <div>
            ...
            <p
              className={`text-xs flex items-center gap-1 ${presence === 'busy' ? 'text-red-600' : 'text-green-600'
                }`}
            >
              <ShieldCheck size={12} color="currentColor" /> {presence === 'busy' ? 'In Call' : 'Active'}
            </p>
          </div>
        </div >
      ...
      < div className="bg-yellow-50 px-4 py-2 text-xs text-center text-yellow-800 flex items-center justify-center gap-1 border-b border-yellow-100" >
        <Lock size={12} color="currentColor" /> Messages are end - to - end encrypted.No one outside this chat can read them.
      </div >
          ...
          messages.map((msg) => (
            ...
            >
              <div
                ...
                className={`max-w-[75%] rounded-2xl p-3 shadow-sm cursor-pointer transition-all active:scale-95 ${msg.sender === 'me'
                    ...
                    <div className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
                      ...
                      <div className="bg-gradient-to-r from-violet-500 to-purple-600 p-3 flex items-center justify-between">
                        <div className="flex items-center gap-2 text-white">
                          <Gamepad2 size={20} color="currentColor" />
      ...
      <div className="p-4 bg-white border-t border-gray-100 flex flex-col gap-2">
        {replyingTo && (
          <div className="flex items-center justify-between bg-gray-50 p-2 rounded-lg border-l-4 border-primary text-xs w-full">
            ...
            <button onClick={() => setReplyingTo(null)} className="p-1"><X size={16} color="#9CA3AF" /></button>
          </div>
        )}
            ...
            <div className="h-60 flex flex-col">
              {pickerTab === 'emoji' ? (
                <>
                  ...
                  <div className="flex-1 overflow-y-auto p-2">
                    <div className="grid grid-cols-7 gap-1">
                      {EMOJI_CATEGORIES[activeEmojiCategory].filter((e: string) => e !== 'mw' && e !== 'Mw').map((emoji: string, idx: number) => (
                        <button
                          key={idx}
                ...
                <>
                  {/* GIF Categories */}
                  <div className="flex overflow-x-auto p-2 bg-gray-100 gap-2 scrollbar-hide no-scrollbar">
                    {(Object.keys(GIF_CATEGORIES) as (keyof typeof GIF_CATEGORIES)[]).map((cat) => (
                      <button
                        key={cat}
                  ...
                  <div className="flex-1 overflow-y-auto p-2">
                    <div className="grid grid-cols-2 gap-2">
                      {GIF_CATEGORIES[activeGifCategory].map((url: string, idx: number) => (
                        <button
                          key={idx}
            ...
            {showGameSelector && (
              <div className="absolute bottom-16 left-0 bg-white rounded-xl shadow-2xl border border-gray-100 p-3 w-[280px] z-20 animate-in slide-in-from-bottom-2">
                <div className="flex justify-between items-center mb-2 px-1">
                  ...
                  <button onClick={() => setShowGameSelector(false)}><X size={16} color="#9CA3AF" /></button>
                </div>
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      className="flex-1 flex flex-col items-center gap-1 p-2 bg-purple-50 hover:bg-purple-100 rounded-lg text-purple-700 transition-colors"
                    >
                      <Film size={20} color="currentColor" />
                      <span className="text-xs">Gallery</span>
                    </button>
                  </div>
                </div>
