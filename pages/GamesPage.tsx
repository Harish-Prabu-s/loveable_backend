    ...
    return (
      <div className="fixed inset-0 z-50 bg-black flex flex-col">
        ...
        <div className="flex-1 flex flex-col items-center justify-center bg-gray-800 text-white p-8 text-center overflow-hidden">
          ...
          {activeGame.id === 'fruit' && <FruitSlash onGameOver={(w: 'me' | 'opponent' | 'draw') => handleGameOver(w)} />}
          {activeGame.id === 'candy' && <CandyMatch onGameOver={(w: 'me' | 'opponent' | 'draw') => handleGameOver(w)} />}
          {activeGame.id === 'carrom' && <CarromBoard onGameOver={(w: 'me' | 'opponent' | 'draw') => handleGameOver(w)} />}
          {activeGame.id === 'ludo' && <LudoBoard onGameOver={(w: 'me' | 'opponent' | 'draw') => handleGameOver(w)} />}
