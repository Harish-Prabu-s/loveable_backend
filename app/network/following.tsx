...
export default function FollowingScreen() {
    ...
    const handleUnfollow = (profile: any) => {
        Alert.alert(
            'Unfollow',
            `Unfollow ${profile.display_name || 'this user'}?`,
            [
                { text: 'Cancel', style: 'cancel' },
                {
                    ...
                    onPress: async () => {
                        try {
                            ...
                            setUsers(prev => prev.filter(u => u.user !== profile.user));
                        } catch {
        ...
        <SafeAreaView style={styles.container}>
            <View style={styles.header}>
                ...
                <Text style={styles.headerTitle}>
                    Following {users.length > 0 ? `(${users.length})` : ''}
                </Text>
                <View style={{ width: 40 }} />
            </View>
            ...
            {loading ? (
                ...
                <FlatList
                    ...
                    ListEmptyComponent={
                        <View style={styles.centerContainer}>
                            <MaterialCommunityIcons name="account-heart-outline" size={64} color="#334155" />
                            <Text style={styles.emptyText}>Not following anyone yet.</Text>
                            <Text style={styles.emptyHint}>Discover people from the Explore tab!</Text>
                        </View>
                    }
                />
            )}
...
const styles = StyleSheet.create({
    ...
    unfollowBtn: {
        ...
        borderRadius: 20, borderWidth: 1.5, borderColor: '#8B5CF6',
    },
    unfollowBtnText: { color: '#8B5CF6', fontSize: 12, fontWeight: '700' },
});
